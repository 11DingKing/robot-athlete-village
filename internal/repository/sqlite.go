...
func (s *SQLite) CreateStay(ctx context.Context, did, rid int64, key string, now time.Time) (domain.Stay, error) {
	...
	if _, err = tx.ExecContext(ctx, "UPDATE rooms SET occupied=occupied+1,version=version+1 WHERE id=? AND version=?", rid, ver); err != nil {
		return domain.Stay{}, err
	}
...
func (s *SQLite) BookingCapacity(ctx context.Context, sid int64) (int, int, error) {
	var status string
	var capacity int
	if err := s.db.QueryRowContext(ctx, "SELECT t.status,v.capacity FROM training_slots t JOIN venues v ON v.id=t.venue_id WHERE t.id=?", sid).Scan(&status, &capacity); err != nil {
		return 0, 0, appErr.ErrNotFound
	}
	if status != "open" {
		return 0, 0, appErr.ErrConflict
	}
	var active int
	if err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM bookings WHERE slot_id=? AND status IN ('held','confirmed')", sid).Scan(&active); err != nil {
		return 0, 0, err
	}
